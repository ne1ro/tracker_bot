defmodule TrackerBot.Pivotal do
  ...
  def list_people(project_id) do
    with {:ok, %{body: people}} <- get("/my/people?project_id=#{project_id}") do
      people
    end
  end
  ...
  def list_stories(project_id) do
    params =
      URI.encode_query(%{
        ...
        envelope: "true",
        created_after:
          Timex.now() |> Timex.shift(days: -@ttl) |> Timex.format!("{ISO:Extended:Z}")
      })

    with {:ok, %{body: %{"data" => stories}}} <- get("/projects/#{project_id}/stories?#{params}") do
      stories
    end
  end
